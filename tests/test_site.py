...
SHAPED_SUMMARY = ("## In plain language\n\n"
                  "### What matters most\n\n- A $40M budget vote\n\n"
                  "### Routine business\n\n- A board reappointment\n\n"
                  "*This summary was generated by an AI model from the agenda.*\n")


def test_index_upcoming_card_shows_lead_open_and_groups_collapsed() -> None:
    html = render_index([], [_upcoming(summary_md=SHAPED_SUMMARY)], TODAY)
    assert "A $40M budget vote" in html
    assert "<details><summary>Everything else on this agenda</summary>" in html
    # the groups sit inside the disclosure, the lead ahead of it
    assert html.index("A $40M budget vote") < html.index("Everything else on this")
    assert html.index("Everything else on this") < html.index("A board reappointment")


def test_index_upcoming_card_keeps_the_ai_disclaimer_visible() -> None:
    """The disclaimer must not end up buried inside the collapsed groups."""
    html = render_index([], [_upcoming(summary_md=SHAPED_SUMMARY)], TODAY)
    assert "generated by an AI model" in html
    collapsed = html.split("Everything else on this agenda</summary>", 1)[1]
    body, _, after = collapsed.partition("</details>")
    assert "generated by an AI model" not in body
    assert "generated by an AI model" in after


def test_index_upcoming_card_renders_an_ungrouped_summary_open() -> None:
    """Pre-2026-09-09 summaries have no `###` groups; show them as before."""
    html = render_index([], [_upcoming(summary_md="## In plain language\n\n- flat\n")],
                        TODAY)
    assert "- flat" not in html and "flat" in html      # rendered as markdown
    assert "Everything else on this agenda" not in html


def test_meeting_page_renders_the_whole_summary_open() -> None:
    """An archive page is a record; nothing about it needs collapsing."""
    html = render_meeting_page(_meeting(summary_md=SHAPED_SUMMARY))
    assert "A $40M budget vote" in html
    assert "A board reappointment" in html
    assert "Everything else on this agenda" not in html
